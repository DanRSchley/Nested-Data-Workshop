...
# Conceptual illustration: what parallel trends looks like when it holds vs. fails
t_pts   <- 0:5
cut_t   <- 2.5

df_pt_obs <- bind_rows(
  # Valid case: same pre-trend slope
  tibble(t = t_pts, group = "Treated",
         y = 50 + 2 * t_pts + ifelse(t_pts > cut_t, 10, 0),
         case = "Parallel trends HOLDS (valid DiD)"),
  tibble(t = t_pts, group = "Control",
         y = 40 + 2 * t_pts,
         case = "Parallel trends HOLDS (valid DiD)"),
  # Failure case: treated group already on steeper trajectory
  tibble(t = t_pts, group = "Treated",
         y = 50 + 4 * t_pts + ifelse(t_pts > cut_t, 4, 0),
         case = "Parallel trends FAILS (invalid DiD)"),
  tibble(t = t_pts, group = "Control",
         y = 40 + 2 * t_pts,
         case = "Parallel trends FAILS (invalid DiD)")
)

df_counter <- bind_rows(
  tibble(t = t_pts[t_pts > cut_t], group = "Counterfactual\n(unobserved)",
...
ggplot() +
  geom_vline(aes(xintercept = cut_t), linetype = "dotted", color = "gray50",
             linewidth = 1.0) +
  geom_line(data = df_pt_obs,
            aes(x = t, y = y, color = group, linetype = group),
            linewidth = 1.3) +
  geom_line(data = df_counter,
            aes(x = t, y = y, color = group, linetype = group),
            linewidth = 1.0) +
  ...
  labs(
    title    = "Parallel trends: valid DiD (left) vs. invalid DiD (right)",
    subtitle = "Orange dashed = what treated group would have done absent treatment.\nLeft: pre-slopes are equal — DiD captures only the jump.\nRight: treated group was already diverging — DiD conflates policy effect with pre-existing momentum.",
    x        = "Time period",
    y        = "Outcome",
