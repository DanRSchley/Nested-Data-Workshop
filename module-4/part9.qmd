...
Whether an observation is missing depends on **observed variables** in the dataset, but not on the missing value itself after conditioning on those variables. Modern methods (FIML via `lmer`, multiple imputation) are valid under MAR.
...
**Plain-language meaning:** Multiple imputation and FIML via `lmer` are only valid if the data are MAR — missing for reasons explainable by observed variables — or MCAR. If dropout is driven by the unobserved outcome trajectory, these methods reduce but cannot eliminate bias.
