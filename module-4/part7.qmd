...
When the DGP generates binary or count data, a linear mixed model imposes a structure that does not match the outcome's constraints. Predicted probabilities can fall outside [0, 1]. Residuals cannot be normally distributed. The Gaussian linear model is misspecified for binary and count outcomes — it does not respect the bounded, discrete DGP. Predicted probabilities can fall outside [0, 1], the mean-variance relationship is wrong, and standard errors and effect interpretations can be misleading. A GLMM is the appropriate primary model.
...
The chart below shows what happens when you fit a linear mixed model to a binary outcome versus a logistic mixed model. Both models are fit to the same data: 60 customers, binary purchase outcome, one predictor (loyalty program × baseline purchase tendency). The problem is immediate: the linear model produces **predicted probabilities below 0** for customers with low baseline purchase tendency, and potentially above 1 for customers with high tendency and program membership. These are impossible predicted probabilities — and they make confidence intervals and counterfactual predictions unreliable.

```{r lm-vs-glmm-illustration, echo=FALSE, fig.width=10, fig.height=4.2, message=FALSE, warning=FALSE}
set.seed(99)
n_ill  <- 200
x_pred <- rnorm(n_ill, 0, 1.2)   # purchase tendency (centered)
treat  <- rep(c(0, 1), each = n_ill / 2)
prob   <- plogis(-0.3 + 0.8 * x_pred + 0.6 * treat)
y_bin  <- rbinom(n_ill, 1, prob)

df_ill <- tibble(x_pred, treat = factor(treat, labels = c("Control", "Loyalty program")),
                 y_bin)

# Fit LMM (linear probability model) and GLMM
m_lm   <- lm(y_bin ~ x_pred * treat, data = df_ill)
m_glm  <- glm(y_bin ~ x_pred * treat, data = df_ill, family = binomial)

df_pred <- expand_grid(
  x_pred = seq(-3, 3, length.out = 100),
  treat  = factor(c("Control", "Loyalty program"))
) |>
  mutate(
    pred_lm  = predict(m_lm,  newdata = tibble(x_pred, treat)),
    pred_glm = predict(m_glm, newdata = tibble(x_pred, treat), type = "response")
  )

# LMM predictions panel
p_lm <- ggplot() +
  geom_point(data = df_ill, aes(x = x_pred, y = y_bin, color = treat),
             alpha = 0.3, size = 1.3) +
  geom_line(data = df_pred, aes(x = x_pred, y = pred_lm, color = treat),
            linewidth = 1.2) +
  geom_hline(yintercept = c(0, 1), linetype = "dashed", color = "#e63946",
             linewidth = 0.9) +
  annotate("rect", xmin = -3.2, xmax = -0.8, ymin = -0.25, ymax = -0.01,
           fill = "#fee8e8", alpha = 0.7) +
  annotate("text", x = -2.0, y = -0.15, label = "Impossible:\nprediction < 0",
           size = 2.6, color = "#c0392b", hjust = 0.5) +
  scale_color_manual(values = c("#aec6e8", "#1e3a5f"), name = NULL) +
  scale_y_continuous(limits = c(-0.30, 1.30),
                     breaks = c(0, 0.25, 0.5, 0.75, 1)) +
  labs(title    = "(A) Linear model (LPM): predictions outside [0, 1]",
       subtitle = "Red dashed lines = valid probability bounds. Shaded region = impossible predictions.",
       x = "Purchase tendency (centered)", y = "Predicted probability") +
  theme_minimal(base_size = 11)

# GLMM predictions panel
p_glm <- ggplot() +
  geom_point(data = df_ill, aes(x = x_pred, y = y_bin, color = treat),
             alpha = 0.3, size = 1.3) +
  geom_line(data = df_pred, aes(x = x_pred, y = pred_glm, color = treat),
            linewidth = 1.2) +
  geom_hline(yintercept = c(0, 1), linetype = "dashed", color = "#2d6a4f",
             linewidth = 0.9) +
  scale_color_manual(values = c("#aec6e8", "#1e3a5f"), name = NULL) +
  scale_y_continuous(limits = c(-0.30, 1.30),
                     breaks = c(0, 0.25, 0.5, 0.75, 1)) +
  labs(title    = "(B) Logistic model (GLMM): predictions stay within [0, 1]",
       subtitle = "S-curve keeps all predictions in the valid range. Odds ratio interpretation applies.",
       x = "Purchase tendency (centered)", y = "Predicted probability") +
  theme_minimal(base_size = 11)
...
broom.mixed::tidy(m_binary, effects = "fixed", exponentiate = TRUE, conf.int = TRUE) |>
  ...
  knitr::kable(digits = 3,
    caption = "Fixed effects as odds ratios: loyalty program increases purchase odds")
```
...
AIC(m_poisson, m_negbin) |>
  knitr::kable(caption = "Model comparison: Poisson vs. Negative Binomial for app opens")
```
...
|---|---|---|---|---|
| 0/1 event | Made a purchase this week | `glmer(..., family = binomial)` | logit | Odds ratios |
| Counts (no overdispersion) | App opens per week (low variance) | `glmmTMB(..., family = poisson)` | log | Rate ratios |
| Counts (overdispersed) | Transactions per month (bursty) | `glmmTMB(..., family = nbinom2)` | log | Rate ratios |
| Counts (excess zeros) | Overdraft events (many zero months) | `glmmTMB(..., ziformula = ~1, family = nbinom2)` | log | Rate ratios |
