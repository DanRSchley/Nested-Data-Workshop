...
tibble(
  ...
  knitr::kable(digits = 3,
               caption = "All three approaches recover the true ATT ≈ 10. Clustered SEs exceed naive SEs because productivity shocks are correlated within firms over time.")
```
...
df_piv <- expand_grid(firm = 1:n_f, year = 1:n_y) |>
  mutate(
    ...
    # R&D: endogenous — high-quality firms invest more
    rd      = 20 + 2 * fq + 3 * tax_inc + rnorm(n(), 0, 5),
    # Patents: causal effect of R&D = 0.5; also depends on quality
    patents = 5 + 0.5 * rd + 1.5 * fq + rnorm(n(), 0, 3),
    firm    = factor(firm),
    year    = factor(year)
  )
```
...
tibble(
  ...
  knitr::kable(digits = 4,
               caption = "OLS + FE overestimates the R&D effect because time-varying firm quality (fq) remains a confounder. IV recovers the true coefficient of 0.5.")
```
